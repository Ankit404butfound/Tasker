            ...
            public void onClick(View v) {
                ...
                if (task_title.matches("")) {
                    ...
                    if (remind_year == 0 && remind_month == 0 && remind_day == 0) {
                        ...
                        long delay = notifyJob.calculate_time(remind_year, remind_month, remind_day, remind_hour, remind_minute);

                        new NetworkManager().execute(Globals.schedule_notification_url(delay, task_title, task_desp));

                        if (Globals.RESULT_FROM_ASYNC_TASK == "false"){
                            Toast.makeText(Add_Work.this, "Error scheduling task, server might be down...", Toast.LENGTH_LONG).show();
                        }
